...
int main(int argc, const char *argv[]) {
    ...
    std::string npu_files_path = "./npu_files/Z-Image-Turbo";
    int seed                   = 42;
    int image_H                = 1024;
    int image_W                = 768;
    int step                   = 4;
    ...
    try {
        // 2. Define the options and default values (matches your manual logic)
        po::options_description desc("Allowed options");
        desc.add_options()
            ("help,h", "Produce help message")
            
            // These were the original Boost options from your terminal screenshot
            ("device,d", po::value<std::string>()->default_value("npu2"), "Device type, npu1 or npu2")
            
            // These are your new options, now integrated into Boost
            ("weights_path", po::value<std::string>(&weights_path), "Path to model weights directory")
            ("npu_files_path", po::value<std::string>(&npu_files_path), "Path to NPU files directory")
            ("seed", po::value<int>(&seed)->default_value(42), "Random seed")
            ("image_H", po::value<int>(&image_H)->default_value(1024), "Image height")
            ("image_W", po::value<int>(&image_W)->default_value(768), "Image width")
            ("step", po::value<int>(&step)->default_value(4), "Number of generation steps")
            ("prompt", po::value<std::string>(&my_prompt)->default_value("a beautiful landscape"), "Text prompt")
            ("output_path", po::value<std::string>(&output_path)->default_value("./images/output.png"), "Path to save the generated image");
